...
function f(D1L::DTPSAD{N, T}, D2L::DTPSAD{N, T}, D3L::DTPSAD{N, T}, 
            Q1L::DTPSAD{N, T}, Q2L::DTPSAD{N, T}, Q1k::DTPSAD{N, T}, Q2k::DTPSAD{N, T}) where {N, T}
    distparam = [
    ...
    # when any of the parameters is DTPSAD type, convert the particle coordinates to DTPSAD type
    beam = Beam(DTPSAD.(Pts1), energy=1.0e9, current=200.0, mass=m_p, charge=1.0)
    ...
    n = 1
    new_emit= zeros(DTPSAD{N, T}, n+1, 3)
    for i in 1:n
        # println("Turn: ", i)
        if i == 1
            get_emittance!(beam)
...
# Uncomment the following lines to use Enzyme for automatic differentiation
#########################################################################################################################
# using JuTrack
# # using PyCall
# # np = pyimport("numpy")
