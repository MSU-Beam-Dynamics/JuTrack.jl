...
function RFCavityPass!(r_in::Vector{CTPS{T, TPS_Dim, Max_TPS_Degree}}, le, nv, freq, h, lag, 
    ...
    if le == 0
        for c in 1:num_particles
            ...
            r_in[5] += -nv * sin(2.0 * pi * freq * ((r_in[6] - lag) / C0 - (h / freq - T0) * nturn) - philag)
            # end
        end
    else
        for c in 1:num_particles
            ...
            # if !isnan(r6[1])
                ...
                # nturn = 0
                # r_in[5] = tminus(r_in[5], tmult(nv, tsin(tminus(tmult(2*pi*freq, tdiv(tminus(r_in[6], lag), C0)), philag))))
                r_in[5] += -nv * sin(2 * pi * freq * ((r_in[6] - lag) / C0 - (h / freq - T0) * nturn) - philag)
                drift6!(r_in, halflength)
            # end
        end
    end
...
function pass_TPSA!(ele::RFCA, r_in::Vector{CTPS{T, TPS_Dim, Max_TPS_Degree}}, num_particles::Int64) where {T, TPS_Dim, Max_TPS_Degree}
    # ele: RFCA
    ...
    RFCavityPass!(r_in, ele.len, nv, ele.freq, ele.h, ele.lag, ele.philag, 0, T0, num_particles)
    return nothing
end
