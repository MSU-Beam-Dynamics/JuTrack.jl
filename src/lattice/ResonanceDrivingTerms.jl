...
function avedata(ring::Vector{<:AbstractElement{DTPSAD{N, T}}}, dpp::Float64; E0::Float64=3e9, m0::Float64=m_e) where {N, T}
    ...
    beta_has_begin = [beta[end:end,:]; beta]
    alpha_has_begin = [alpha[end:end,:]; alpha]
    mu_has_begin = [[DTPSAD(0.0) DTPSAD(0.0)]; mu]
    dp_has_begin = [dp[end:end,:]; dp]
    ...
    L = zeros(DTPSAD{NVAR(), Float64}, length(long))
    for i in eachindex(long)
        L[i] = ring[long[i]].len
    end
    L2 = [L L]
...
struct DrivingTermsTPSAD{N}
    ...
    h10110::Vector{DTPSAD{N, Float64}}
    h10020::Vector{DTPSAD{N, Float64}}
    h10200::Vector{DTPSAD{N, Float64}}
    h11001::Vector{DTPSAD{N, Float64}}
    h00111::Vector{DTPSAD{N, Float64}}
    h20001::Vector{DTPSAD{N, Float64}}
    h00201::Vector{DTPSAD{N, Float64}}
    h10002::Vector{DTPSAD{N, Float64}}
    h10010::Vector{DTPSAD{N, Float64}}
    h10100::Vector{DTPSAD{N, Float64}}
    h22000::Vector{DTPSAD{N, Float64}}
    h11110::Vector{DTPSAD{N, Float64}}
    h00220::Vector{DTPSAD{N, Float64}}
    h31000::Vector{DTPSAD{N, Float64}}
    h40000::Vector{DTPSAD{N, Float64}}
    h20110::Vector{DTPSAD{N, Float64}}
    h11200::Vector{DTPSAD{N, Float64}}
    h20020::Vector{DTPSAD{N, Float64}}
    h20200::Vector{DTPSAD{N, Float64}}
    h00310::Vector{DTPSAD{N, Float64}}
    h00400::Vector{DTPSAD{N, Float64}}
    dnux_dJx::DTPSAD{N, Float64}
    dnux_dJy::DTPSAD{N, Float64}
    dnuy_dJy::DTPSAD{N, Float64}
end

struct ElementDataTPSAD{N}
    betax::DTPSAD{N, Float64}
    betay::DTPSAD{N, Float64}
    rbetax::DTPSAD{N, Float64}
    rbetay::DTPSAD{N, Float64}
    betax2::DTPSAD{N, Float64}
    betay2::DTPSAD{N, Float64}
    phix::DTPSAD{N, Float64}
    phiy::DTPSAD{N, Float64}
    px::Vector{DTPSAD{N, ComplexF64}}
    py::Vector{DTPSAD{N, ComplexF64}}
    b2L::DTPSAD{N, Float64}
    b3L::DTPSAD{N, Float64}
    s::DTPSAD{N, Float64}
end
...
function computeDrivingTerms(s::Vector{DTPSAD{N, T}}, betax::Vector{DTPSAD{N, Float64}}, betay::Vector{DTPSAD{N, Float64}}, 
    ...
    NumElem = length(s)
    ed = Vector{ElementDataTPSAD{N}}(undef, NumElem)
    ii = im * 1.0
    # Initialize complex variables
    h11001 = h00111 = h20001 = h00201 = h10002 = h10010 = h10100 = 0.0 + 0.0im
    h21000 = h30000 = h10110 = h10020 = h10200 = 0.0+ 0.0im
    ...
    for nE in 1:NumElem
        ...
        etax1 = etax[nE]
        # Fill the element data
        ed[nE] = ElementDataTPSAD{N}(betax1, betay1, sqrt(betax1), sqrt(betay1), betax1^2, betay1^2,
                            phix1, phiy1, [exp(ii * phix1 * j) for j in 1:5], [exp(ii * phiy1 * j) for j in 1:5],
                            b2L, b3L, s[nE])
    ...
    # Create and return the structure of driving terms
    DrivingTermsTPSAD{N}([abs(h21000), real(h21000), imag(h21000)], [abs(h30000), real(h30000), imag(h30000)],
                  [abs(h10110), real(h10110), imag(h10110)], [abs(h10020), real(h10020), imag(h10020)],
                  [abs(h10200), real(h10200), imag(h10200)], [abs(h11001), real(h11001), imag(h11001)],
                  [abs(h00111), real(h00111), imag(h00111)], [abs(h20001), real(h20001), imag(h20001)],
                  [abs(h00201), real(h00201), imag(h00201)], [abs(h10002), real(h10002), imag(h10002)],
...
function juliaRDT(s::Vector{Float64}, betax::Vector{Float64}, betay::Vector{Float64}, 
    phix::Vector{Float64}, phiy::Vector{Float64}, etax::Vector{Float64}, 
    Lista2L::Vector{Float64}, Listb2L::Vector{Float64}, Listb3L::Vector{Float64}, Listb4L::Vector{Float64}, 
    Tunex::Float64, Tuney::Float64, NumElem::Int, Geometric1::Bool, Geometric2::Bool, 
    Chromatic1::Bool, Coupling1::Bool, TuneShifts::Bool, nPeriods::Int)    
...
function juliaRDT(s::Vector{DTPSAD{N, T}}, betax::Vector{DTPSAD{N, T}}, betay::Vector{DTPSAD{N, T}}, 
    phix::Vector{DTPSAD{N, T}}, phiy::Vector{DTPSAD{N, T}}, etax::Vector{DTPSAD{N, T}}, 
    Lista2L::Vector{DTPSAD{N, T}}, Listb2L::Vector{DTPSAD{N, T}}, Listb3L::Vector{DTPSAD{N, T}}, Listb4L::Vector{DTPSAD{N, T}}, 
    Tunex::DTPSAD{N, T}, Tuney::DTPSAD{N, T}, NumElem::Int, Geometric1::Bool, Geometric2::Bool, 
    Chromatic1::Bool, Coupling1::Bool, TuneShifts::Bool, nPeriods::Int) where {N, T}
    flags = RDTflags(Geometric1, Geometric2, Chromatic1, Coupling1, TuneShifts)
    tune = [Tunex, Tuney]
    d = computeDrivingTerms(s, betax, betay, phix, phiy, etax, Lista2L, Listb2L, Listb3L, Listb4L, tune, flags, nPeriods)
    return d
...
function computeRDT(ring::Vector{<:AbstractElement{DTPSAD{N, T}}}, index::Vector{Int}; 
    ...
    dlist = Vector{DrivingTermsTPSAD{NVAR()}}(undef, length(index))
    for ii in eachindex(index)
        FromindexDQSO = cal_FromindexDQSO(indDQSO, index[ii])
        betax_Fromindex = [betax[FromindexDQSO:end]; betax[1:FromindexDQSO-1]]
        betay_Fromindex = [betay[FromindexDQSO:end]; betay[1:FromindexDQSO-1]]
