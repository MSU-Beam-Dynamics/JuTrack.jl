...
Describe one active lattice parameter without mutating the stored lattice.
`index` is the element index in the lattice, `field` is the element field to
replace, and optional `subindices` select an entry of an array field.

Pass the lattice as the first argument when the parameter will be used inside
Enzyme differentiation; this stores a concrete element template and avoids
mutating or rebuilding the full lattice.
...
```
"""
struct LatticeParameter{Field,N,E}
    index::Int
    template::E
    subindices::NTuple{N,Int}
end

function LatticeParameter(index::Integer, field::Symbol, subindices::Integer...)
    return LatticeParameter{field,length(subindices),Nothing}(
        Int(index), nothing, Tuple(Int(i) for i in subindices))
end

function LatticeParameter(line::AbstractVector, index::Integer, field::Symbol,
    subindices::Integer...)
    idx = Int(index)
    template = line[idx]
    return LatticeParameter{field,length(subindices),typeof(template)}(
        idx, template, Tuple(Int(i) for i in subindices))
end

lattice_parameter(index::Integer, field::Symbol, subindices::Integer...) =
    LatticeParameter(index, field, subindices...)
lattice_parameter(line::AbstractVector, index::Integer, field::Symbol,
    subindices::Integer...) = LatticeParameter(line, index, field, subindices...)

@inline parameter_index(param::LatticeParameter) = param.index
@inline parameter_field(::LatticeParameter{Field}) where {Field} = Field
@inline parameter_subindices(param::LatticeParameter) = param.subindices

const LatticeParameterCollection =
    Union{AbstractVector{<:LatticeParameter},Tuple{Vararg{LatticeParameter}}}

_active_element_field(x) = x
_active_element_field(x::AbstractArray) = copy(x)

@generated function _replace_element_field(ele::E, ::Val{Field}, value) where {E<:AbstractElement,Field}
    names = fieldnames(E)
    if !(Field in names)
        return :(throw(ArgumentError("Element type $E has no field $(QuoteNode(Field)).")))
    end
    constructor = Base.typename(E).wrapper
    args = Any[
        name === Field ? :value : :(_active_element_field(getfield(ele, $(QuoteNode(name)))))
        for name in names
    ]
    return :($constructor($(args...)))
end

function with_parameter(ele::AbstractElement, param::LatticeParameter{Field,0}, value) where {Field}
    return _replace_element_field(ele, Val(Field), value)
end

function with_parameter(ele::AbstractElement, param::LatticeParameter{Field,N}, value) where {Field,N}
    field_value = copy(getfield(ele, Field))
    field_value[param.subindices...] = value
    return _replace_element_field(ele, Val(Field), field_value)
end

with_parameter(param::LatticeParameter{Field,N,E}, value) where {Field,N,E<:AbstractElement} =
    with_parameter(param.template, param, value)

with_parameter(param::LatticeParameter{Field,N,Nothing}, value) where {Field,N} =
    throw(ArgumentError("This LatticeParameter has no element template. Call with_parameter(line, param, value), or construct it as LatticeParameter(line, index, field)."))

with_parameter(line::AbstractVector, param::LatticeParameter, value) =
    with_parameter(line[param.index], param, value)

with_parameter(line::AbstractVector, param::LatticeParameter{Field,N,E}, value) where {Field,N,E<:AbstractElement} =
    with_parameter(param.template, param, value)

function _parameter_indices_are_unique(params)
    for i in eachindex(params)
        for j in Iterators.drop(eachindex(params), i)
            parameter_index(params[i]) == parameter_index(params[j]) && return false
        end
    end
    return true
end

function _materialize_lattice_parameters(params::AbstractVector{<:LatticeParameter{Field,N,E}},
    values) where {Field,N,E<:AbstractElement}
    length(params) == length(values) ||
        throw(ArgumentError("params and values must have the same length."))
    _parameter_indices_are_unique(params) ||
        throw(ArgumentError("Templated LatticeParameter vectors must contain each element index only once. Build a changed element manually when multiple fields on one element change together."))
    changed_idx = Vector{Int}(undef, length(params))
    changed_ele = Vector{E}(undef, length(params))
    for i in eachindex(params)
        changed_idx[i] = parameter_index(params[i])
        changed_ele[i] = with_parameter(params[i], values[i])
    end
    return changed_idx, changed_ele
end

function _materialize_lattice_parameter(param::LatticeParameter{Field,N,E},
    value) where {Field,N,E<:AbstractElement}
    changed_idx = Int[parameter_index(param)]
    changed_ele = E[with_parameter(param, value)]
    return changed_idx, changed_ele
end

function changed_elements(line::AbstractVector{<:AbstractElement{T}},
    param::LatticeParameter, value) where {T}
    changed_idx = Int[param.index]
    changed_ele = [with_parameter(line, param, value)]
    return changed_idx, changed_ele
end

function changed_elements(::AbstractVector{<:AbstractElement{T}},
    param::LatticeParameter{Field,N,E}, value) where {T,Field,N,E<:AbstractElement}
    return _materialize_lattice_parameter(param, value)
end

function changed_elements(::AbstractVector{<:AbstractElement{T}},
    params::AbstractVector{<:LatticeParameter{Field,N,E}}, values) where {T,Field,N,E<:AbstractElement}
    return _materialize_lattice_parameters(params, values)
end

function changed_elements(line::AbstractVector{<:AbstractElement{T}},
    params::LatticeParameterCollection, values) where {T}
    length(params) == length(values) ||
        throw(ArgumentError("params and values must have the same length."))
    if allunique(parameter_index, params)
        changed_idx = [parameter_index(params[i]) for i in eachindex(params)]
        changed_ele = [with_parameter(line, params[i], values[i]) for i in eachindex(params)]
        return changed_idx, changed_ele
    end
    changed_idx = Int[]
    changed_ele = AbstractElement{T}[]
    for i in eachindex(params)
        param = params[i]
        slot = findfirst(==(param.index), changed_idx)
        if slot === nothing
            push!(changed_idx, param.index)
            push!(changed_ele, with_parameter(line, param, values[i]))
        else
            changed_ele[slot] = with_parameter(changed_ele[slot], param, values[i])
        end
    end
    return changed_idx, changed_ele
end
...
function changed_elements(line::AbstractVector{<:AbstractElement{T}},
    indices::AbstractVector{<:Integer}, field::Symbol, value,
    subindices::Integer...) where {T}
    params = [LatticeParameter(line, idx, field, subindices...) for idx in indices]
    changed_idx = [parameter_index(params[i]) for i in eachindex(params)]
    changed_ele = [with_parameter(params[i], value) for i in eachindex(params)]
    return changed_idx, changed_ele
end
...
_replace_element_field(::LorentzBoost, ::Val{:angle}, value) = LorentzBoost(value)
_replace_element_field(::InvLorentzBoost, ::Val{:angle}, value) = InvLorentzBoost(value)
