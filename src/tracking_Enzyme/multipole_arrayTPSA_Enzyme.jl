...
function apply_canonical_multipole_kicks(qx::Array{T}, qy::Array{T}, 
                x::Array{T}, y::Array{T}, 
                order::Int, KnL::Float64, skew::Int) where {T}
    # different from multi-particle tracking
    sum_Fx = zeros(Float64, length(x))
    sum_Fy = zeros(Float64, length(x))
    coef = expansion_coefficients(order)  
    
    odd_para = -1
    for i in 0:order
        if odd_para == 1
            sum_Fx = tadd(sum_Fx, tmult(coef[i + 1], tmult(tpow(x, order - i), tpow(y, i))))
            # sum_Fx += coef[i + 1] * x^(order - i) * y^i #xpow[order - i + 1] * ypow[i + 1]
        else
            sum_Fy = tadd(sum_Fy, tmult(coef[i + 1], tmult(tpow(x, order - i), tpow(y, i))))
    ...
    delta_qx = - tmult(KnL, sum_Fy)
    delta_qy = tmult(KnL, sum_Fx)

    qx = tminus(qx, tmult(KnL, sum_Fy))
    qy = tadd(qy, tmult(KnL, sum_Fx))
    # qx -= KnL * sum_Fy
    # qy += KnL * sum_Fx
    return qx, qy, delta_qx, delta_qy
end

function convertSlopesToMomenta(xp::Array{T}, yp::Array{T}, delta::Array{T}) where T
    expandHamiltonian = 0
    # if expandHamiltonian == 1
    #     qx = (1.0 + delta) * xp
    #     qy = (1.0 + delta) * yp
    # else
        denom = tsqrt(tadd(1.0, tadd(tpow(xp, 2), tpow(yp, 2))))
        qx = tdiv(tmult(tadd(1.0, delta), xp), denom)
        qy = tdiv(tmult(tadd(1.0, delta), yp), denom)
...
function convertMomentaToSlopes(qx::Array{T}, qy::Array{T}, delta::Array{T}) where T
    expandHamiltonian = 0
    # if expandHamiltonian == 1
    #     xp = qx / (1.0 + delta)
    #     yp = qy / (1.0 + delta)
    # else
        denom = tpow(tadd(1.0, delta), 2) - tpow(qx, 2) - tpow(qy, 2)
        # denom = (1.0 + delta)^2 - qx^2 - qy^2
        # if denom < 0
        #     warn("Warning: particle acquired undefined slopes when integrating through kick multipole")
        # end
        # denom = sqrt(denom)
        xp = tdiv(qx, tsqrt(denom))
        yp = tdiv(qy, tsqrt(denom))
        # xp = qx / sqrt(denom)
        # yp = qy / sqrt(denom)
    # end
    return xp, yp
...
function offsetBeamCoordinates(x::Array{T}, xp::Array{T}, y::Array{T}, yp::Array{T}, z::Array{T}, delta::Array{T}, 
                    np::Int, dx::T, dy::T, dz::T) where T
    new_x = tadd(x, -dx, tmult(dz, xp))
    new_y = tadd(y, -dy, tmult(dz, yp))
    new_z = tadd(z, tmult(dz, tsqrt(tadd(1.0, tpow(xp, 2), tpow(yp, 2)))))
    return new_x, xp, new_y, yp, new_z, delta
end
...
function rotateBeamCoordinates(x, xp, y, yp, z, delta, np, angle)
    ...
    # end
    x_new = tadd(tmult(x, cos_a), tmult(y, sin_a))
    xp_new = tadd(tmult(xp, cos_a), tmult(yp, sin_a))
    y_new = tadd(tmult(x, -sin_a), tmult(y, cos_a))
    yp_new = tadd(tmult(xp, -sin_a), tmult(yp, cos_a))
...
function integrate_kick_multipole_ordn(x::Array{T}, xp::Array{T}, y::Array{T}, yp::Array{T}, z::Array{T}, delta::Array{T}, 
    dx::Float64, dy::Float64, xkick::Float64, ykick::Float64,
    Po::Float64, rad_coef::Float64, isr_coef::Float64, synch_rad::Int, 
    order::Int, KnL1::Float64, KnL2::Float64, KnL3::Float64, skew::Array{Int},
    n_parts::Int, drift::Float64,
    integration_order::Int, dzLoss, refTilt) where T
    ...
    s  = zeros(Float64, length(z))
    dp = CTPS(delta)
    p = tadd(1.0, delta)
    p = tmult(p, Po)
    temp = tadd(1.0, tmult(delta, delta))
    beta0 = tdiv(p, tsqrt(temp))
    ...
    for i_kick in 0:n_parts-1
        for step in 1:nSubsteps
            if drift != 0.0
                ...
                x = tadd(x, tmult(xp, dsh))
                y = tadd(y, tmult(yp, dsh))
                s = tadd(s, tmult(dsh, tsqrt(tadd(1.0, tadd(tpow(xp, 2), tpow(yp, 2))))))
                # x += xp * dsh
                # y += yp * dsh
            ...
            if synch_rad == 1  && drift !=0.0
                qx = tdiv(qx, tadd(1.0, dp))
                qy = tdiv(qy, tadd(1.0, dp))
                deltaFactor = tpow(tadd(1.0, dp), 2)
                # qx = qx / (1.0 + dp) 
                # qy = qy / (1.0 + dp)
                # deltaFactor = (1.0 + dp)^2

                delta_qx = tdiv(delta_qx, kickFrac[step])
                delta_qy = tdiv(delta_qy, kickFrac[step])
                F2 = tadd(tpow(tdiv(tadd(delta_qx, -xkick), drift), 2), 
                            tpow(tdiv(tadd(delta_qy, ykick), drift), 2))
                # delta_qx /= kickFrac[step]
                # delta_qy /= kickFrac[step]
                # F2 = (delta_qx/drift-xkick/drift)^2 + (delta_qy/drift+ykick/drift)^2

                dsFactor = tmult(tmult(tsqrt(tadd(1.0, tadd(tpow(xp, 2), tpow(yp, 2)))), kickFrac[step]), drift)
                # dsISRFactor = dsFactor*drift/(nSubsteps_float-1.0)
                dsFactor = tmult(tmult(dsFactor, drift), kickFrac[step])
                # dsFactor = sqrt(1.0 + xp^2 + yp^2) * drift * kickFrac[step]
                # dsISRFactor = dsFactor*drift/(nSubsteps_float-1.0)
                # dsFactor *= drift * kickFrac[step]

                # if rad_coef != 0.0
                    dp = tminus(dp, tmult(rad_coef, tmult(deltaFactor, F2, dsFactor)))
                ...
                qx = tmult(qx, tadd(1.0, dp))
                qy = tmult(qy, tadd(1.0, dp))
                # qx = qx * (1.0 + dp)
                # qy = qy * (1.0 + dp)
    ...
    if synch_rad == 1
        p = tmult(Po, tadd(1.0, dp))
        beta1 = tdiv(p, tsqrt(tadd(tpow(p, 2), 1.0)))
        coord5 = tmult(beta1, tadd(tdiv(z, beta0), tmult(2.0, tdiv(s, tadd(beta0, beta1)))))
        # p = Po * (1.0 + dp)
...
function pass_TPSA(x::Array{T}, xp::Array{T}, y::Array{T}, yp::Array{T}, z::Array{T}, delta::Array{T}, 
                    elem::KQUAD, Po::Float64, sigmaDelta2::Float64) where T
    # constants
...
function pass_TPSA(x::Array{T}, xp::Array{T}, y::Array{T}, yp::Array{T}, z::Array{T}, delta::Array{T}, 
    elem::KSEXT, Po::Float64, sigmaDelta2::Float64) where T
# constants
particleMass = 9.1093897e-31 # Electron mass (kg)
particleCharge = 1.60217733e-19 # Electron charge (C)
c_mks = 299792458.0 # Speed of light (m/s)
...
function pass_TPSA(x::Array{T}, xp::Array{T}, y::Array{T}, yp::Array{T}, z::Array{T}, delta::Array{T}, 
    elem::KOCT, Po::Float64, sigmaDelta2::Float64) where T
# constants
particleMass = 9.1093897e-31 # Electron mass (kg)
particleCharge = 1.60217733e-19 # Electron charge (C)
c_mks = 299792458.0 # Speed of light (m/s)
...
function f2(xx)
    ...
    elem = KSEXT(len=L, k2=K, synch_rad=1)
...
println(f2(xx))
# @btime f2(xx)
# println(gradient(Reverse, f2, xx))
@btime gradient(Forward, f2, xx)