...
function track!(rin, beam::Beam, tm::TransferMap4D, tempx, temppx, tempy, num_macro, lost_flags::Array{Int64,1})

    for j in 1:num_macro
        if isone(lost_flags[j])
            continue
        end
...
#function track!(beam::Beam, tm::TransferMap4D)
    lost_flags = beam.lost_flag
    track!(r_in, beam, tm, beam.temp1, beam.temp2, beam.temp3, num_macro, lost_flags)
end


#se commento tempy e r6[3] è tutto okay
function track!(rin, beam::Beam, tm::Inverse_TransferMap4D, tempx, temppx, tempy, num_macro, lost_flags::Array{Int64,1})
...
function track!(rin, beam::Beam, tm::TransferMap4DChrom, temp1, temp2, temp3, sinphi, cosphi, num_macro, lost_flags::Array{Int64,1}) #where T
...
#function track!(beam::Beam, tm::TransferMap4DChrom)
    lost_flags = beam.lost_flag
    track!(r_in, beam, tm,  beam.temp1, beam.temp2, beam.temp3, beam.temp4, beam.temp5, num_macro, lost_flags)
end
